...
$("#loginsubmit").click(function () {
    ...
    if (flag) {
        ...
        var _from = $("#formlogin [name=from]").val();
        var _fromId = $("#formlogin [name=fromId]").val();
        $.ajax({
            type: "POST",
            url: "http://weige.taotao.com/rest/customer/user/doLogin?r=" + Math.random(),
            contentType: "application/x-www-form-urlencoded; charset=utf-8",
            data: {username:_username,password:_password,from:_from,fromId:_fromId},
            dataType : "json",
            error: function () {
            ...
            success: function (result) {
                if (result) {
                    ...
                    if (obj.status == 200) {
                    	if (obj.from != ""){
                    		obj.success = obj.from;
                    	}else{
                    		obj.success = "http://weige.taotao.com/rest/customer/index";
                    	}
                        var isIE = !-[1,];
                        if (isIE) {
