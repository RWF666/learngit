...
function checkPin(option) {
    ...
    if (!namestate || nameold != pin) {
        if (nameold != pin) {
            ...
            $.ajax({
            	url : "http://weige.taotao.com/rest/customer/user/check/"+escape(pin)+"/1?r=" + Math.random(),
            	dataType : "json",
...
function sendMobileCode() {
    ...
    // 检测手机号码是否存在
    $.ajax({
    	url : "http://weige.taotao.com/rest/customer/user/check/"+mobile+"/2?r=" + Math.random(),
    	dataType : "json",
    	success : function(result) {
            if (!result) {
...
function sendMobileCode1() {
    ...
    // 检测手机号码是否存在
    $.ajax({
    	url : "weige.taotao.com/rest/customer/user/check/"+mobile+"/2?r=" + Math.random(),
    	dataType : "json",
    	success : function(result) {
            if (!result) {
                $('#phone1').removeClass().addClass("text");
